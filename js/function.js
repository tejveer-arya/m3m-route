        ...
        patterns: {
            youtube: {
                ...
                id: function(url) {
                    ...
                    var match = url.match(/[?&]v=([^&]+)/); // Regular YouTube URL
                    if (match) {
                        return match[1];
                    }
                    match = url.match(/youtube\.com\/embed\/([^?]+)/); // Embedded YouTube URL
                    if (match) {
                        return match[1];
                    }
                    return null;
    ...
    navLinks.forEach(link => {
        link.addEventListener("click", function () {
            if (window.innerWidth <= 991) {
                slickNavMenu.style.display = "none";
            }
        });
    });

    // Show menu again when resizing above 991px
    window.addEventListener("resize", function () {
        if (window.innerWidth > 991) {
            slickNavMenu.style.display = "";
        }
    });
});
